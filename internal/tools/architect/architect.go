...
func RegisterTasks() {
	...
	tool := reg.RegisterTool("architect", "Architectural modeling and rendering tools")

	renderCat := tool.AddCategory("render", "Rendering operations")

	renderCat.AddTask("demo", "Run the architectural rendering demo", func(ctx *registry.TaskContext) error {
...
// Helper functions adapted from examples/canvas_demo.go
func drawPlan(c *canvas.Canvas, offX, offY, w int) {
	// Title
...
func drawElevation(c *canvas.Canvas, offX, offY int) {
...
func drawData(c *canvas.Canvas, offX, offY int) {
